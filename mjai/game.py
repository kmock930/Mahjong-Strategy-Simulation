...
class Game:
    def __init__(self, agents, logs_dir="./logs"):
        self.agents = agents
        self.logs_dir = Path(logs_dir)
        self.logs_dir.mkdir(parents=True, exist_ok=True)
    ...
    def start_game(self):
        next_state = None
        for run_idx in range(10):  # Example loop, adjust as needed
            next_state = self._internal_run(run_idx, next_state)
            if next_state is None:
                break
    ...
    def _internal_run(self, run_idx, next_state):
        # Implement the internal run logic here
        logger.info(f"Running game iteration {run_idx}")
        
        # Example logic for a game round
        ...
        if self.current_round >= 10:  # Example condition to stop the game
            return None
        return next_state
...
# Ensure this is at the end of your game.py file
if __name__ == "__main__":
    # Example usage
    from agent1 import SimpleAgent
    from RandomAgent import RandomAgent

    agents = [SimpleAgent(), RandomAgent(), SimpleAgent(), RandomAgent()]
    game = Game(agents)
    game.start_game()